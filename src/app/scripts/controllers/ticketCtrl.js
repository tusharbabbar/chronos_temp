    ...
    TicketTagApi.get({ticket_id: $routeParams.id},
      ...
      $scope.saveTicketDetails = function() {
        ...
        if($scope.data['ownerTeam'].length){
          if ($scope.data.ticket.owner_details){
        ...
        data.fabric_practice_id = $scope.data.fabric_practice_id ?  $scope.data.ticket.kyc_info ? $scope.data.fabric_practice_id != $scope.data.ticket.kyc_info.fabric_practice_id ?
